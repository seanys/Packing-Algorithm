...
class GA(object):
    ...
    def getLength(self,poly_list):
        length=PolyListProcessor.packingLength(poly_list,self.history_index_list,self.history_length_list,self.width,NFPAssistant=self.NFPAssistant)
        return length
...
class SA(object):
    ...
    def run(self):
        initial_length=PolyListProcessor.packingLength(self.cur_poly_list,self.history_index_list,self.history_length_list,self.width)
        ...
        while self.temp_now>self.temp_end:
            ...
            cur_length=PolyListProcessor.packingLength(self.cur_poly_list,self.history_index_list,self.history_length_list,self.width,NFPAssistant=self.NFPAssistant)

            # 在某个温度下进行一定次数的寻找 
            for i in range(self.loop_times): 
                self.newPolyList()

                new_length=PolyListProcessor.packingLength(self.new_poly_list,self.history_index_list,self.history_length_list,self.width,NFPAssistant=self.NFPAssistant)
