...
class Compaction(object):
    ...
    def run(self):
        ...
        for i in range(N):
            ...
            a[row+3][i*2+2],b[row+3]=-1,self.H[i]-self.WIDTH  # -yi>=Hi-Width
        ...
        # 求解计算结果
        result=sovleLP(a,b,c)

        # # 将其转化为坐标
        placement_points=[]
        for i in range(len(result)//2):
            placement_points.append([result[i*2+1],result[i*2+2]])
    ...
    def getOverlapConstrain(self,i,j):
        ...
        # 1: (y2-y1)*xj+(x1-x2)*yj+x2*y1-x1*y2>0 
        # 2: (Y2-Y1)*xj+(X1-X2)*yj+X2*Y1-X1*Y2+(xi-Xi)*(Y1-Y2)+(yi-Yi)*(X2-X1)+>0
        # 3: (Y2-Y1)*xj+(X1-X2)*yj+X2*Y1-X1*Y2+(Y1-Y2)*xi+(X2-X1)*yi-Xi*(Y1-Y2)-Yi*(X2-X1)>0
        # 4: (Y1-Y2)*xi+(X2-X1)*yi+(Y2-Y1)*xj+(X1-X2)*yj>-X2*Y1+X1*Y2+Xi*(Y1-Y2)+Yi*(X2-X1)
...
if __name__ == "__main__":
    polys=getConvex(num=3)
    # print(BottomLeftFill(1000,polys).polygons)

    blf = pd.read_csv("/Users/sean/Documents/Projects/Packing-Algorithm/record/blf.csv")
    Compaction(1000,json.loads(blf["polys"][0]))
    # ILSQN(poly_list)
